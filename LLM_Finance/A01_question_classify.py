...
os.environ["CUDA_VISIBLE_DEVICES"]="3"  # 设置只使用GPU:3

company_file_dir = './data/pdf_to_company.csv'
company_file = pd.read_csv(company_file_dir, delimiter=",", header=0)
company_list = set(company_file["公司名称"].tolist())  # 提取【公司名称】列的值为列表 -> set

new_question_file = pd.read_csv("./data/00_all_questions.csv", delimiter = ",", header = 0)
...
with open('./data/01_question_classify.csv', 'w', newline='', encoding = 'utf-8') as f:
    csvwriter = csv.writer(f)
    csvwriter.writerow(['问题id','问题','答案','分类'])

    for i in tqdm(range(len(new_question_file)), desc="Processing Questions"):
        
        cur_question = new_question_file.iloc[i]['问题']  # 获取问题内容
        prompt = prompt_template.format(cur_question)         # 生成 Prompt
        ...
        if '招股说明书' in response and '股票数据库' not in response:
            ...
            for company_name in company_list:
                if company_name in cur_question:
                    type = 'Text'
                    break
        ...
        csvwriter.writerow([
            new_question_file.iloc[i]['问题id'],
            new_question_file.iloc[i]['问题'],
            response,
            type
        ])